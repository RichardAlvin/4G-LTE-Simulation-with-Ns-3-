...
main (int argc, char *argv[])
{
  ...
  for(uint16_t i = 0; i < numberOfEnbs; i++){
	  if(i == 0){
		  Vector position(10,18,0);
		  positionAlloc->Add(position);
	  }else if(i == 1){
		Vector position(25,18,0);
		positionAlloc->Add(position);
	  }else if(i == 2){
		Vector position(20,28,0);
		positionAlloc->Add(position);
          }else{
		Vector position(22,5,0);
		positionAlloc->Add(position);
	  }
  }

  MobilityHelper emobility;
  emobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
  emobility.SetPositionAllocator(positionAlloc);
  emobility.Install(enbNodes);
...
//  mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
 ...
 //                                   "LayoutType", StringValue ("SecondFirst"));
  //mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
 // mobility.Install (enbNodes);
 //
  MobilityHelper amobility; 
  amobility.SetPositionAllocator ("ns3::GridPositionAllocator",
				...
				"GridWidth", UintegerValue(4),
				"LayoutType", StringValue("RowFirst"));
  amobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  amobility.Install(sgw);
  amobility.Install(pgw);
  amobility.Install(remoteHost);
